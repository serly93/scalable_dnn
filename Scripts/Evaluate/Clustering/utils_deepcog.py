...
def load_model_custom(city, CELL, alpha):
    model_path = f'../../../../oracle-data/serly/TMC_data/Trained_models/{city}/{damage}/Models/mymodel_{CELL}.h5' #TO DO change the cell for multiple cells
    if damage == 'capacity_forecasting':
...
def load_model_deepcog(model_path, damage, alpha):
...
def preprocess_cluster_data(cluster_cells, all_cells, output_directory, city, lookback, nr, data_type="train"):
    ...
    with a subset of cells as input and all cells in the cluster as targets.
